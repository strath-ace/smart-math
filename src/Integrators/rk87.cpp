...
template<class T>
rk87<T>::rk87(const dynamics::base_dynamics<T> *dyn, const double tol, const double multiplier, const double minstep_events) : base_integrator<T>("Runge Kutta 4-5 variable step time", dyn)
{
	   /** sanity checks **/
	if(tol<=0.0)
...
int rk87<T>::integrate(const double &ti, const double &tend, const int &nsteps, const std::vector<T> &x0, std::vector<T> &xfinal) const{
    ...
    int n = x0.size();
    std::vector<T> x(x0), xbar(x0), xtemp(x0), k1(x0), k2(x0), k3(x0), k4(x0), k5(x0), k6(x0), k7(x0), k8(x0), k9(x0);
	std::vector<T> k10(x0), k11(x0), k12(x0), k13(x0);
